...
/**
 * Enhanced Trading Signal Generator - Fixes price fetching and improves signal quality
 * 
 * This version integrates with real-time price data and fixes the NaN/0.000000 issues
 */
...
const CONFIG = {
  ...
  priceDataFile: path.join(__dirname, '../data/enhanced-live-data.json'),
...
function loadCurrentPrices() {
  try {
    if (!fs.existsSync(CONFIG.priceDataFile)) {
      console.log('⚠️  No price data file found, will fetch from API');
      return {};
    }
    
    const priceData = JSON.parse(fs.readFileSync(CONFIG.priceDataFile, 'utf8'));
    const prices = {};
    
    // Extract prices from the data structure
    if (priceData.tokens && Array.isArray(priceData.tokens)) {
      priceData.tokens.forEach(token => {
        if (token.symbol && token.price) {
          prices[token.symbol.toUpperCase()] = parseFloat(token.price);
        }
      });
    }
    
    console.log(`📊 Loaded ${Object.keys(prices).length} current prices`);
    return prices;
  } catch (error) {
...
// Fetch missing prices from CoinGecko API
async function fetchMissingPrices(symbols, existingPrices) {
  const missingSymbols = symbols.filter(symbol => 
    !existingPrices[symbol] || existingPrices[symbol] === 0
  );
  
  if (missingSymbols.length === 0) {
    console.log('✅ All prices already available');
  ...
  try {
    ...
    const response = await axios.get('https://api.coingecko.com/api/v3/simple/price', {
      params: {
        ids: missingSymbols.map(s => s.toLowerCase()).join(','),
        vs_currencies: 'usd'
      },
      timeout: 10000
    });
    ...
    Object.entries(response.data).forEach(([id, data]) => {
      ...
      if (data.usd) {
        newPrices[symbol] = data.usd;
        console.log(`   ${symbol}: $${data.usd.toFixed(6)}`);
      }
    });
...
// Generate trading signal with proper price data
function generateTradingSignal(alert, mlPrediction, portfolioSize, currentPrices) {
  ...
  // Get current price - first try from currentPrices, then from alert metrics
  ...
  if (!currentPrice || currentPrice === 0) {
    console.log(`⚠️  Skipping ${alert.symbol}: No price data available`);
    return null;
  }
  ...
  const riskPercent = risk > 0 ? (risk / currentPrice * 100) : 0;
  ...
  const signal = {
    ...
    // Risk management
    riskPerTrade: riskPercent.toFixed(2) + '%',
    rewardPotential: ((takeProfit - currentPrice) / currentPrice * 100).toFixed(2) + '%',
    riskRewardRatio: riskRewardRatio.toFixed(2),
    maxLossUSD: (positionSize * riskPercent / 100).toFixed(2),
  ...
  console.log(`✅ Generated signal for ${alert.symbol}: $${entryPrice.toFixed(4)} (${confidence}% confidence)`);
  return signal;
}
...
function calculateTakeProfit(currentPrice, predictedChange) {
  ...
  if (!predictedChange || predictedChange === 0) {
    return currentPrice * 1.15; // Default 15% target
  }
...
function assessOverallRisk(signals) {
  ...
  const highRiskSignals = signals.filter(s => {
    const riskPercent = parseFloat(s.riskPerTrade);
    return riskPercent >= 15;
  });
...
// Main execution
async function main() {
  console.log('📈 Enhanced Trading Signal Generator');
  ...
  try {
    ...
    let currentPrices = loadCurrentPrices();
    ...
    currentPrices = await fetchMissingPrices(symbols, currentPrices);
    ...
    for (const alert of alerts.slice(0, 20)) { // Process recent alerts
      ...
      const signal = generateTradingSignal(alert, mlPrediction, portfolioSize, currentPrices);
    ...
    if (signals.length === 0) {
      console.log('⚠️  No valid signals generated - check price data and confidence thresholds');
      return { success: false, error: 'No valid signals generated' };
    }
    ...
    console.log('\n✅ Enhanced Trading Signal Generator completed successfully!');
    ...
    console.log(`💵 Sample signal: ${signals[0].symbol} @ $${signals[0].entryPrice} (${signals[0].riskPerTrade} risk)`);
    ...
    console.error('❌ Enhanced Trading Signal Generator failed:', error.message);
